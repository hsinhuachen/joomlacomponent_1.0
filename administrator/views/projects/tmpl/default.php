...
if ($saveOrder){
	$saveOrderingUrl = 'index.php?option=com_onion_project&task=projects.saveOrderAjax&tmpl=component';
...
?>
<form action="<?php echo JRoute::_('index.php?option=com_onion_project&view=projects'); ?>" method="post" name="adminForm" id="adminForm">
		...
		<div class="clearfix">
			...
			<div class="js-stools-container-list hidden-phone hidden-tablet shown" style="">
				<select name="filter_category" class="inputbox" onchange="this.form.submit()">
		            ...
		            <?php echo JHtml::_('select.options', $cateList, 'id', 'title', $this->state->get('filter.category'));?>
		        </select>
			</div>
			<?php
				/* total */
				...
				$db->setQuery("select count(*) from #__onion_projects" . $countWhere);
				$count = $db->loadResult();
		...
		<tbody>
			<?php
                ...
                foreach ($this->items as $i => $item){
                    $link = JRoute::_( 'index.php?option=' . $option . '&task=project.edit&id=' . $item->id );
                ...
                <tr>
                    ...
                    <td class="center order">                  
						...
						<?php if ($canChange && $saveOrder) : ?>
							<input type="text" name="order[]" size="5" value="<?php echo $item->ordering; ?>" class="width-20 text-area-order " style="display: none" />
						<?php endif; ?>
                    </td>
	...
	<div>
		<input type="hidden" name="target" value="#__onion_projects" />
